...
resource "null_resource" "backend" {
    triggers = {
      instance_id = module.backend.id   # this triggers every time instance is created
    }
...
resource "aws_ec2_instance_state" "backend" {
  ...
  state = "stopped"
  # stop the server when null provision is completed because it will prepare the server to fully ready and running( full fledged backend server)
