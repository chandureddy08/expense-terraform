...
resource "null_resource" "frontend" {
    triggers = {
      instance_id = module.frontend.id   # this triggers every time instance is created
    }
...
resource "aws_ec2_instance_state" "frontend" {
  ...
  state = "stopped"
  # stop the server when null provision is completed because it will prepare the server to fully ready and running( full fledged frontend server)
